...
declare namespace prom {
    /**
     * This will perform a POST request using `fetch`.
     * @param url URL to post data to
     * @param body JSON or string data to post
     * @param headers Optional headers
     */
    declare function postData(url: string, body: {}, headers?: {}): Promise<void>;

    /**
     * This will perform a POST request using `fetch` with the provided authorization header.
     * @param url URL to post data to
     * @param body JSON or string data to post
     * @param authorization Authorization header
     */
    declare function postWithAuth(url: string, body: {}, authorization: string): Promise<void>;

    /**
     * This will POST the current state of metrics to the VictoriaMetrics at `url`.
     * `url` argument should point to the endpoint `api/v1/import/prometheus`.
     * @param url URL of Victoria Metrics.
     * @param authorization Authorization header
     */
    declare function pushToVM(url: string, authorization: string): Promise<void>;

    /** Counter that will automatically include and manage a "server" label. */
    declare var ServerCounter: typeof Counter;

    /** Gauge that will automatically include and manage a "server" label. */
    declare var ServerGauge: typeof Gauge;
    ...
    declare class AggregatorRegistry extends Registry {
        /**
     ...
     * General metric type
     */
    declare type Metric<T extends string = string> =
        | Counter<T>
        | Gauge<T>
        | Summary<T>
        | Histogram<T>;
     ...
     */
    declare type Aggregator = "omit" | "sum" | "first" | "min" | "max" | "average";
    ...
    /**
     * ------------------------------------
     * This feature was removed by Telokis.
     * ------------------------------------
     * @deprecated
     */
     ...
     */
    declare function linearBuckets(start: number, width: number, count: number): number[];
     ...
     * @return An array with the requested number of elements
     */
    declare function exponentialBuckets(start: number, factor: number, count: number): number[];
     ...
     */
    declare function collectDefaultMetrics(config?: DefaultMetricsCollectorConfiguration): void;
     ...
     */
    declare function validateMetricName(name: string): boolean;
}
